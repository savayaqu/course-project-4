        ...
        $array = [
            ...
            $this->mergeWhen($isThis || $isAdmin, [
                ...
                $this->mergeWhen($isAdmin, [
                    ...
                    'complaintsAbout'      => $this->whenLoaded ('complaintsAbout',
                        fn() => $this->when($this->complaintsAbout->isNotEmpty(), fn() => ComplaintResource::collection($this->complaintsAbout))
                    ),
                ]),
                'complaintsFromCount' => $this->whenCounted('complaintsFrom', fn($count) => $this->when($count, $count)),
                'complaintsFrom'      => $this->whenLoaded ('complaintsFrom',
                    fn() => $this->when($this->complaintsFrom->isNotEmpty(), fn() => ComplaintResource::collection($this->complaintsFrom))
                ),
                'warningsCount' => $this->whenCounted('warnings', fn($count) => $this->when($count, $count)),
                'warnings'      => $this->whenLoaded ('warnings',
                    fn() => $this->when($this->warnings->isNotEmpty(), fn() => WarningResource::collection($this->warnings))
                ),
                ...
                'albums'      => $this->whenLoaded ('albums',
                    fn() => $this->when($this->albums->isNotEmpty(), fn() => AlbumResource::collection($this->albums))
                ),
                'albumsViaAccessCount' => $this->whenCounted('albumsViaAccess', fn($count) => $this->when($count, $count)),
                'albumsViaAccess'      => $this->whenLoaded ('albumsViaAccess',
                    fn() => $this->when($this->albumsViaAccess->isNotEmpty(), fn() => UserResource::collection($this->albumsViaAccess))
                ),
                'picturesCount' => $this->whenCounted('pictures', fn($count) => $this->when($count, $count)),
                'pictures'      => $this->whenLoaded ('pictures',
                    fn() => $this->when($this->pictures->isNotEmpty(), fn() => PictureResource::collection($this->pictures))
                ),
            ])
        ];
